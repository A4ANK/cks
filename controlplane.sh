...
kubeadm init --apiserver-advertise-address 192.168.99.102 --kubernetes-version=${KUBE_VERSION} --ignore-preflight-errors=NumCPU --node-name "controlplane" --skip-token-print --pod-network-cidr 192.168.0.0/16

mkdir -p ~/.kube
sudo cp -i /etc/kubernetes/admin.conf ~/.kube/config

### CNI
kubectl apply -f https://raw.githubusercontent.com/A4ANK/cks/main/calico.yaml
